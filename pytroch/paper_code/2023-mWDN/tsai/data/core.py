...
# %% auto 0
__all__ = ['TSCategorize', 'TSRegression', 'TSForecasting', 'get_tsimage_dls', 'NumpyTensor', 'ToNumpyTensor',
           'TSTensor',
...
from fastai.data.transforms import (RandomSplitter, ItemGetter, CategoryMap,
...
class NumpyTensor(TensorBase):
    ...
    @property
    def data(self):
        return cast(self, Tensor)

    def __repr__(self):
        if self.ndim > 0:
            return f'NumpyTensor(shape:{tuple(self.shape)}, device={self.device}, dtype={self.dtype})'
        else:
            return f'NumpyTensor([{self.data}], device={self.device}, dtype={self.dtype})'

    def show(self, ax=None, ctx=None, title=None, **kwargs):
        if self.ndim == 0:
            return str(self.data)
        elif self.ndim != 2:
            self = type(self)(to2d(self))
...
class TSTensor(TensorBase):
    ...
    def __repr__(self):
        if self.ndim > 3:
            ...
            return f'TSTensor([{self.data}], device={self.device}, dtype={self.dtype})'
...
class TSLabelTensor(NumpyTensor):
    def __repr__(self):
        if self.ndim == 0:
            ...
            return f'TSLabelTensor(shape:{tuple(self.shape)}, device={self.device}, dtype={self.dtype})'
...
class TSMaskTensor(NumpyTensor):
    def __repr__(self):
        if self.ndim == 0:
            ...
            return f'TSMaskTensor(shape:{tuple(self.shape)}, device={self.device}, dtype={self.dtype})'
...
class ToInt(Transform):
    ...
    def encodes(self, o):
        return np.asarray(o).astype(np.float32).astype(np.int64)
...
class NumpyDataset():
    def __init__(self, X, y=None, types=None):
    ...
    def __getitem__(self, idx):
        if self.types is None:
            ...
            return (self.types[0](self.X[idx]), self.types[1](self.y[idx])) if self.y is not None else (
                self.types[0](self.X[idx]))
...
class NoTfmLists(TfmdLists):
    ...
    def subset(self, i, **kwargs):
        return type(self)(self.items, splits=self.splits[i], split_idx=i, do_setup=False, types=self.types,
                          **kwargs)

    def __getitem__(self, it):
        if hasattr(self.items, 'oindex'):
            return self.items.oindex[self._splits[it]]
        else:
            return self.items[self._splits[it]]
...
class NumpyDatasets(Datasets):
    ...
    def __init__(self, X=None, y=None, items=None, tfms=None, tls=None, n_inp=None, dl_type=None, inplace=True,
                 **kwargs):
        ...
        if tls is None:
            ...
            if tfms is None:
                ...
                lts = [NoTfmLists if t is None else TSTfmdLists if getattr(t, 'vectorized', None) else TfmdLists for t
                       in self.tfms]

            self.tls = L(lt(item, t, **kwargs) for lt, item, t in zip(lts, items, self.tfms))
            if len(self.tls) > 0 and len(self.tls[0]) > 0:
                self.typs = [type(tl[0]) if isinstance(tl[0], torch.Tensor) else self.typs[i] for i, tl in
                             enumerate(self.tls)]
            self.ptls = L(
                [typ(stack(tl[:])) for i, (tl, typ) in enumerate(zip(self.tls, self.typs))]) if inplace else self.tls
        else:
            self.tls = tls
            if len(self.tls) > 0 and len(self.tls[0]) > 0:
                self.typs = [type(tl[0]) if isinstance(tl[0], torch.Tensor) else self.typs[i] for i, tl in
                             enumerate(self.tls)]
            self.ptls = L(
                [typ(stack(tl[:])) for i, (tl, typ) in enumerate(zip(self.tls, self.typs))]) if inplace and len(
                tls[0]) != 0 else tls
    ...
    def new_empty(self):
        return type(self)(tls=[tl.new_empty() for tl in self.tls], n_inp=self.n_inp, inplace=self.inplace)
...
class TSDatasets(Datasets):
    ...
    def __init__(self, X=None, y=None, items=None, sel_vars=None, sel_steps=None, tfms=None, tls=None, n_inp=None,
                 dl_type=None,
                 inplace=True, **kwargs):
        ...
        if tls is None:
            ...
            self.tls = L(lt(item, t, **kwargs) for lt, item, t in zip(lts, items, self.tfms))
            if len(self.tls) > 0 and len(self.tls[0]) > 0:
                self.typs = [type(tl.items[0]) if isinstance(tl.items[0], torch.Tensor) else self.typs[i] for i, tl in
                             enumerate(self.tls)]
            if self.inplace and (tfms is None or tfms == [None] * len(self.tls)):
                ...
                self.ptls = L([typ(stack(tl[:]))[..., self.sel_vars, self.sel_steps] if (
                            i == 0 and self.multi_index) else typ(stack(tl[:])) \
            ...
            self.ptls = L([typ(stack(tl[:]))[..., self.sel_vars, self.sel_steps] if (
                        i == 0 and self.multi_index) else typ(stack(tl[:])) \
                           for i, (tl, typ) in enumerate(zip(self.tls, self.typs))]) if inplace and len(
                tls[0]) != 0 else tls
            self.no_tfm = False
    ...
    def __getitem__(self, it):
        if self.inplace or self.no_tfm:
            ...
            return tuple([typ(stack(ptl[it]))[..., self.sel_vars, self.sel_steps] if (
                        i == 0 and self.multi_index) else typ(stack(ptl[it])) \
    ...
    def subset(self, i):
        if is_indexer(i):
            return type(self)(tls=L([tl.subset(i) for tl in self.tls]), inplace=self.inplace, tfms=self.tfms,
                              ...
                              splits=None if self.splits is None else self.splits[i],
                              split_idx=i)
        else:
            ...
            return type(self)(*self[i], inplace=True, tfms=None,
                              sel_vars=self.sel_vars, sel_steps=self.sel_steps, splits=splits,
                              split_idx=ifnone(self.split_idx, 1))

    def _new(self, X, y=None, **kwargs):
        return type(self)(X, y=y, sel_vars=self.sel_vars, sel_steps=self.sel_steps, tfms=self.tfms,
                          inplace=self.inplace,
                          do_setup=False, **kwargs)

    def new_empty(self):
        return type(self)(tls=[tl.new_empty() for tl in self.tls], sel_vars=self.sel_vars, sel_steps=self.sel_steps,
...
class NumpyDataLoader(TfmdDL):
    ...
    def __init__(self, dataset, bs=64, shuffle=False, drop_last=False, num_workers=0, verbose=False, do_setup=True,
        ...
        for nm in _batch_tfms:
            if nm == 'after_batch' and kwargs.get('batch_tfms', None) is not None:
                kwargs[nm] = Pipeline(listify(kwargs.get('batch_tfms')))
            else:
        ...
        super().__init__(dataset, bs=bs, shuffle=shuffle, drop_last=drop_last, num_workers=num_workers, verbose=verbose,
                         do_setup=do_setup, **kwargs)
    ...
    def __len__(self):
        if self.n == 0:
            return 0
        elif self.partial_n is None:
            return super().__len__()
        return self.partial_n // self.bs + (0 if self.drop_last or self.partial_n % self.bs == 0 else 1)

    @delegates(plt.subplots)
    def show_batch(self, b=None, ctxs=None, max_n=9, nrows=3, ncols=3, figsize=None, unique=False, sharex=True,
                   sharey=False, decode=False,
        ...
        if ctxs is None: ctxs = get_grid(max_n, nrows=nrows, ncols=ncols, figsize=figsize, sharex=sharex, sharey=sharey,
                                         **kwargs)
        if show_title:
    ...
    def show_dist(self, figsize=None, color=None, **kwargs):
        ...
        yb = b[i:][0]
        if color == "random":
            color = random_shuffle(L(mcolors.CSS4_COLORS.keys()))
        elif color is None:
            color = ['m', 'orange', 'darkblue', 'lightgray']
    ...
    def d(self):
        ...
        yb = b[i:]
        if len(yb[0].shape[1:]) == 0:
            return None
        elif len(yb[0].shape[1:]) == 1:
            return yb[0].shape[-1]
        else:
            return list(yb[0].shape[1:])

    @property
    def cat(self):
        return hasattr(self, "vocab")
...
class NumpyDataLoaders(DataLoaders):
    ...
    def show_dist(self, figsize=None, **kwargs):
        self.loaders[0].show_dist(figsize=figsize, **kwargs)
    ...
    def decoder(self, o):
        ...
        if o.ndim <= 1:
            ...
            return L([self.decodes(oi) for oi in o])
    ...
    @classmethod
    def from_dsets(cls, *ds, path='.', bs=64, num_workers=0, batch_tfms=None, device=None, shuffle_train=True,
                   drop_last=True,
        ...
        loaders = [
            cls._dl_type(d, bs=b, num_workers=num_workers, batch_tfms=batch_tfms, weights=w, partial_n=n, sampler=s,
                         sort=sort, vocab=vocab, **k) \
...
def get_best_dl_params(dl, n_iters=10, num_workers=[0, 1, 2, 4, 8], pin_memory=[True, False], prefetch_factor=[2, 4, 8],
                       return_best=True,
                       verbose=True):
    ...
    try:
        ...
        if not num_workers:
            best_nw = nw
        elif isinstance(num_workers, Integral):
            best_nw = num_workers
        else:
            best_time = np.inf
            for _nw in num_workers:
                ...
                for i, _ in enumerate(dl):
                    if i == n_iters - 1:
                        t = timer.stop() / (i + 1)
                        pv(f'   num_workers: {_nw:2}  pin_memory: {pm!s:^5}  prefetch_factor: {pf:2}  -  time: {1_000 * t / n_iters:8.3f} ms/iter',
                           verbose)
                        if t < best_time:
                            best_nw = _nw
                            best_time = t
        ...
        if not pin_memory:
            best_pm = pm
        elif isinstance(pin_memory, bool):
            best_pm = pin_memory
        else:
            best_time = np.inf
        ...
        if best_nw == 0:
            best_pf = 2
        elif not prefetch_factor:
            best_pf = pf
        elif isinstance(prefetch_factor, Integral):
            best_pf = prefetch_factor
        else:
            best_time = np.inf
...
def get_best_dls_params(dls, n_iters=10, num_workers=[0, 1, 2, 4, 8], pin_memory=[True, False],
                        prefetch_factor=[2, 4, 8],
                        return_best=True, verbose=True):
    for i in range(len(dls.loaders)):
        try:
            print(f'\nDataloader {i}\n')
            dls.loaders[i] = get_best_dl_params(dls.loaders[i], n_iters=n_iters, num_workers=num_workers,
                                                pin_memory=pin_memory,
                                                prefetch_factor=prefetch_factor, return_best=return_best,
                                                verbose=verbose)
        except KeyboardInterrupt:
            pass
    return dls
...
def get_ts_dls(X, y=None, splits=None, sel_vars=None, sel_steps=None, tfms=None, inplace=True,
    ...
    if weights is not None:
        ...
        weights = [weights[split] if i == 0 else None for i, split in
                   enumerate(splits)]  # weights only applied to train set
