...
x = torch.randn(batch_size, seq_length, input_dim).to(device)  # (batch_size, seq_length, feature_dim)
...
# def uncertainity_estimate(x, model, num_samples, l2, N=1):
#     outputs = np.hstack([model(x).cpu().detach().numpy() for i in range(num_samples)])  # todo - 这里不对 # n번 inference, output.shape = [20, N]
...
# mean_preds, std_preds = uncertainity_estimate(x, model, iters_uncertainty, lengthscale)
...
y_pred = mean_preds[0, :, 0].cpu().numpy()  # 选择第一个批次的预测结果
y_std = std_preds[0, :, 0].cpu().numpy()  # 选择第一个批次的预测不确定性

# 计算置信区间
conf_95_upper_np = conf_95_upper[0, :, 0].cpu().numpy()
conf_95_lower_np = conf_95_lower[0, :, 0].cpu().numpy()
conf_75_upper_np = conf_75_upper[0, :, 0].cpu().numpy()
conf_75_lower_np = conf_75_lower[0, :, 0].cpu().numpy()
