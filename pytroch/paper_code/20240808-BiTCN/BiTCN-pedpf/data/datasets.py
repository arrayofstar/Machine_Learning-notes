...
class uci_electricity(torchdata.Dataset):
    ...
    def get_data(self):
        ...
        df = pd.read_csv('../data/uci_electricity/LD2011_2014.txt', sep=';', parse_dates=[0],
                         dtype='float32', decimal=',', index_col=[0])  # infer_datetime_format=True,
        ...
        Xt, Yt = X.unfold(-2, self.dim_maxseqlen, 1).permute(0, 1, 3, 2), Y.unfold(-2, self.dim_maxseqlen, 1).permute(0,
                                                                                                                      1,
                                                                                                                      3,
                                                                                                                      2)
...
class kaggle_favorita(torchdata.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        y = self.Y[
            self.index[idx] + self.dim_inputseqlen:self.index[idx] + self.dim_outputseqlen + self.dim_inputseqlen]
    ...
    def get_data(self):
        ...
        df_Y = df[['unit_sales']]
        df_X = df.drop(columns=['unit_sales', 'transactions_missing_lagged', 'oil_price_missing_lagged',
                                'unit_sales_missing_lagged', 'transactions_lagged', 'oil_price_lagged'])
        # Convert dataframe to numpy and reshape to [series x dates x features] format   
        ...
        if self.mode == 'train':
            ...
            idx = index[(index['date'] <= self.validate_maxdate) & (index['date'] > self.train_maxdate)][
                'index'].to_numpy()
            self.index = torch.from_numpy(idx)
        elif self.mode == 'test':
    ...
    def get_data(self):
        ...
        if self.mode == 'train':
            self.df_index = df_index[
                (df_index['date'] <= self.train_maxdate) & (df_index['date'] >= self.train_mindate)]
        elif self.mode == 'validate':
            self.df_index = df_index[
                (df_index['date'] <= self.validate_maxdate) & (df_index['date'] > self.train_maxdate)]
        elif self.mode == 'test':
