...
def calc_metrics(yhat, y, quantiles):
    ...
    q = 4
    print(
        f"         RMSE/NRMSE/ND/MAPE/sMAPE loss: {df['RMSE'][q]:.2f}/{df['NRMSE'][q]:.2f}/{df['ND'][q]:.3f}/{df['MAPE'][q]:.3f}/{df['sMAPE'][q]:.3f}")
    print(
        f"         p10/p50/p90/mp50 loss: {df['QuantileLoss'][0]:.3f}/{df['QuantileLoss'][4]:.3f}/{df['QuantileLoss'][8]:.3f}/{df['QuantileLoss'].mean():.3f}")
...
# Read experiment csv
def read_table(filename, error=None):
    for x in range(0, 10):  # todo - 这里的代码比较冗余
        try:
