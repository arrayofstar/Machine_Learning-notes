...
class Dataset_ETT_hour(Dataset):
    ...
    def __read_data__(self):
        ...
        border1s = [0, 12 * 30 * 24 - self.seq_len, 12 * 30 * 24 + 4 * 30 * 24 - self.seq_len]  # mf-划分训练集验证集和测试集
        ...
        df_stamp = df_raw[['date']][border1:border2]  # mf-时间列
        ...
        print(f"data_x的数据类型为：{type(self.data_x)}")  # mf-改
        print(f"data_x的数据维度为：{self.data_x.shape}")  # mf-改
    ...
    def __getitem__(self, index):
        ...
        s_begin = index
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len - self.pred_len
        r_end = r_begin + self.label_len + self.pred_len
