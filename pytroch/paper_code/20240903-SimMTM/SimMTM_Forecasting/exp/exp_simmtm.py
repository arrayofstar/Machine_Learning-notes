...
class Exp_SimMTM(Exp_Basic):
    ...
    def pretrain_one_epoch(self, train_loader, model_optim, model_scheduler):
        ...
        self.model.train()
        for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(train_loader, total=len(train_loader), desc="train_loader")):
            ...
            # to device
            batch_x = batch_x.float().to(self.device)  # mf-无掩码的原始数据
            batch_x_om = batch_x_om.float().to(self.device)  # mf-无掩码和有掩码*3之后cat的数据
            batch_x_mark = batch_x_mark.float().to(self.device)  # mf-无掩码的时间序列列
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            train_loss = np.average(train_loss)
            vali_loss = self.vali(vali_loader, criterion, 'vali_loader')
            test_loss = self.vali(test_loader, criterion, 'test_loader')
    ...
    def vali(self, vali_loader, criterion, describe):
        total_loss = []

        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(vali_loader, total=len(vali_loader), desc=describe)):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                batch_x_mark = batch_x_mark.float().to(self.device)
    ...
    def test(self):
        ...
        self.model.eval()
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(tqdm(test_loader, total=len(test_loader), desc="test_loader")):
                batch_x = batch_x.float().to(self.device)
                batch_y = batch_y.float().to(self.device)
                batch_x_mark = batch_x_mark.float().to(self.device)
