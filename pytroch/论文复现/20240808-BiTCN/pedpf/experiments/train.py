...
experiment_dir = '../experiments/' + dataset_name  # 'experiments/' + dataset_name
...
while table[table['in_progress'] == -1].isnull()['score'].sum() > 0:
    ...
    for epoch in range(epochs):
        ...
        model, loss_train[epoch], _, _, _, _ = loop(model, training_set, optimizer, batch_size, id_samples_train,
                                                    train=True, metrics=True, scaling=scaling)
        _, loss_validate[epoch], yhat_tot, y_tot, x_tot, df_validate = loop(model, validation_set, optimizer,
                                                                            batch_size, id_samples_validate,
                                                                            train=False, metrics=True, scaling=scaling)
