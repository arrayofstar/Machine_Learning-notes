...
if __name__ == '__main__':
    ...
    # x = torch.rand(500, 10, 1).to(device)
    x = torch.rand(128, 200, 3).to(device)
    batch_size = x.shape[0]
    input_size = x.shape[2]
    encoder = EncoderRNN(input_size=input_size, hidden_size=128, num_grulstm_layers=1, batch_size=batch_size).to(device)
    decoder = DecoderRNN(input_size=input_size, hidden_size=128, num_grulstm_layers=1, fc_units=16, output_size=1).to(device)
    net_gru_dilate = Net_GRU(encoder, decoder, 10, device).to(device)
    print(net_gru_dilate)
    out = net_gru_dilate(x)
    print(out.shape)